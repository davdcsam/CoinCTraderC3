...
namespace Trade
{
    ...
    public class PlaceOrders
    {
        ...
        public PlaceOrders(Robot _robot, double lotSize, double stopLoss, double takeProfit, string labelIdentifier, string symbol = null)
        {
            _cbot = _robot;
            ...
            Symbol = symbol ?? _cbot.Symbol.Name;
        }
        ...
        protected bool BuyComposite(double targetPrice)
        {
            ...
            {
                TradeResult tradeResult = _cbot.PlaceStopOrder(TradeType.Buy, Symbol, LotSize, targetPrice, LabelIdentifier, StopLoss, TakeProfit);

                if (tradeResult.IsSuccessful) { return true; }
            }
            else
            {
                TradeResult tradeResult = _cbot.PlaceLimitOrder(TradeType.Buy, Symbol, LotSize, targetPrice, LabelIdentifier, StopLoss, TakeProfit);
        ...
        protected bool SellComposite(double targetPrice)
        {
            ...
            {
                TradeResult tradeResult = _cbot.PlaceStopOrder(TradeType.Sell, Symbol, LotSize, targetPrice, LabelIdentifier, StopLoss, TakeProfit);

                if (tradeResult.IsSuccessful) { return true; }
            }
            else
            {
                TradeResult tradeResult = _cbot.PlaceLimitOrder(TradeType.Sell, Symbol, LotSize, targetPrice, LabelIdentifier, StopLoss, TakeProfit);
